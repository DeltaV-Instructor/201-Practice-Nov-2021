...
function SpaceShipMallProductFactory(productName, imagePath, lsProductsShown, lsProductsClickedOn){
  this.productName = productName;
  this.imagePath = imagePath;
  this.timesProductImageShown = lsProductsShown || 0;
  this.timesClickedOn = lsProductsClickedOn || 0;
...
let localStorageJson = localStorage.getItem('savedProducts');

if(localStorageJson){
  //we have the string now we can parse it
  let parsedProductString = JSON.parse(localStorageJson);

  for(let p = 0; p < parsedProductString.length; p++){
    console.log('running');
    new SpaceShipMallProductFactory(parsedProductString[p].productName,
      parsedProductString[p].imagePath,
      parsedProductString[p].lsProductsClickedOn
    );
  }
  // console.log('local storage', parsedProductString);
  //get local storage out of local storage and make them into space products else
} else {
  ...
  for(let i = 0; i < productImages.length; i++){
    productImages[i].addEventListener('click', function(){
      ...
      if(rounds >= dynamicRound){
        localStorage.setItem('savedProducts', JSON.stringify(SpaceShipMallProductFactory.allProductsArray));
        console.log('poop');
        getChartData();
      }
      runGenerateNewRound();
